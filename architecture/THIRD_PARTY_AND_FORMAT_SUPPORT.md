...
|---|---|---|---|---|
| LightTable layered PNG, manifest v1-v8 | Yes; v1-v8 read | Native layers, vectors, flow/positioned text, fonts, masks, styles, patterns, grades, preserved-source records, v4 layer-local raster bounds, v5 bounded semantic derived previews, v6 external system-font references, v7 authored color settings and v8 document resolution according to schema version | Save writes v8 in a PNG-compatible container; system-font metadata/fingerprint is retained without silently embedding installed font bytes | Private `LTBLDOC1` footer and typed manifest; not a public PNG extension standard |
