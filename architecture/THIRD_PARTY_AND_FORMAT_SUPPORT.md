...
|---|---|---|---|---|
| LightTable layered PNG, manifest v1-v5 | Yes; v1-v5 read | Native layers, vectors, flow/positioned text, fonts, masks, styles, patterns, grades, preserved-source records, v4 layer-local raster bounds and v5 bounded semantic derived previews according to schema version | Save writes v5 in a PNG-compatible container | Private `LTBLDOC1` footer and typed manifest; not a public PNG extension standard |
