...
|---|---|---|---|---|
| LightTable layered PNG, manifest v1-v6 | Yes; v1-v6 read | Native layers, vectors, flow/positioned text, fonts, masks, styles, patterns, grades, preserved-source records, v4 layer-local raster bounds, v5 bounded semantic derived previews and v6 external system-font references according to schema version | Save writes v6 in a PNG-compatible container; system-font metadata/fingerprint is retained without silently embedding installed font bytes | Private `LTBLDOC1` footer and typed manifest; not a public PNG extension standard |
