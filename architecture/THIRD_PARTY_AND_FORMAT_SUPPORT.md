...
| pdf-lib | 1.17.1 | MIT | Lazy bounded one-page flattened and hybrid raster-underlay/native-text PDF writer |
...
- **Export**: writes a deliverable. Quick export is PNG; the PDF preflight can
  write one flattened page or a compatible hybrid raster-underlay/native-text page.
...
| PDF 1.7 / ISO 32000-1 and PDF 2.0 / ISO 32000-2:2020 | Yes; first page through lazy PDF.js preview | Imported as one 300-ppi raster layer while the original PDF remains an immutable preserved source; canonical positioned-text and display-list contracts are not connected to production parsing yet | Native LightTable save preserves the source; PDF preflight exports one flattened page or, for a fail-closed compatible topmost text suffix, a raster underlay with searchable Type0/CIDFontType2 text | Password handling, page selection, multipage documents, semantic import and general vector/group/text interleaving remain gated; the preview must not be presented as editable PDF structure |
