...
const appendJoin = (
  ...
  const previousOuter = offset(point, previousNormal, half * side);
  const nextOuter = offset(point, nextNormal, half * side);

  if (stroke.join === 'round') {
    appendArc(values, point, multiplyScalar(previousNormal, side), multiplyScalar(nextNormal, side), turn > 0 ? 1 : -1, half, tolerance);
    return;
  }
  if (stroke.join === 'miter') {
    ...
    const miterLength = denominator > EPSILON ? half / denominator : Number.POSITIVE_INFINITY;
    if (miterLength <= half * Math.max(1, stroke.miterLimit)) {
...
const appendRun = (values: number[], run: StrokeRun, stroke: VectorStroke, tolerance: number) => {
  ...
  for (let index = 0; index < segmentCount; index += 1) {
    ...
    const leftStart = offset(start, normal, half);
    const rightStart = offset(start, normal, -half);
    const leftEnd = offset(end, normal, half);
    const rightEnd = offset(end, normal, -half);
  ...
  for (let index = firstJoin; index < lastJoin; index += 1) {
    ...
    appendJoin(values, points[index % points.length], directions[previousIndex], directions[nextIndex], stroke, tolerance);
  }
  ...
  if (stroke.cap === 'square') {
    ...
    const startExtended = offset(start, firstDirection, -half);
    const endExtended = offset(end, lastDirection, half);
    appendTriangle(values, offset(start, firstNormal, half), offset(start, firstNormal, -half), offset(startExtended, firstNormal, half));
    appendTriangle(values, offset(start, firstNormal, -half), offset(startExtended, firstNormal, -half), offset(startExtended, firstNormal, half));
    appendTriangle(values, offset(end, lastNormal, half), offset(end, lastNormal, -half), offset(endExtended, lastNormal, half));
    appendTriangle(values, offset(end, lastNormal, -half), offset(endExtended, lastNormal, -half), offset(endExtended, lastNormal, half));
  } else if (stroke.cap === 'round') {
    const firstNormal = { x: -firstDirection.y, y: firstDirection.x };
    const lastNormal = { x: -lastDirection.y, y: lastDirection.x };
    appendArc(values, start, firstNormal, multiplyScalar(firstNormal, -1), 1, half, tolerance);
    appendArc(values, end, multiplyScalar(lastNormal, -1), lastNormal, 1, half, tolerance);
  }
};
...
export const buildStrokeTriangleGeometry = (
  ...
  for (const subpath of geometry.subpaths) {
    for (const run of strokeRuns(subpath, stroke.dash, stroke.dashOffset)) {
      appendRun(values, run, stroke, geometry.key.toleranceBucket);
    }
  }
