...
export const numericIncrement = (
  value: number, direction: -1 | 1, step: number,
  modifiers: { readonly shiftKey?: boolean; readonly altKey?: boolean }
) => value + direction * step * (modifiers.shiftKey ? 10 : modifiers.altKey ? .1 : 1);

export function parseBoundedNumber(draft: string, min: number, max: number) {
