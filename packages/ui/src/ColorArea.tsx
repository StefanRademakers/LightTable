...
export interface ColorAreaProps {
  ...
  onChange: (value: ColorAreaValue, handle?: InteractionHandle) => void;
...
export function ColorArea({ hue, value, tabIndex = -1, disabled = false, ...interactionProps }: ColorAreaProps) {
  ...
  return <div className="ui-color-area" data-ui-component="color-area" role="slider"
    ...
    onPointerDown={event => {
      ...
      pointer.current = event.pointerId; pointerSession.current = interaction.begin();
    ...
    onKeyUp={event => { if (event.key.startsWith('Arrow')) { event.stopPropagation(); finishKeyboard(); } }} onBlur={finishKeyboard}>
