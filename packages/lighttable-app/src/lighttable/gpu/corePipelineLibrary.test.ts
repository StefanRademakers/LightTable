...
describe('getCorePipelineBundle', () => {
  it('reuses immutable pipelines for one device and canvas format', () => {
    ...
    expect(device.createRenderPipeline).toHaveBeenCalledTimes(13);
  ...
  it('separates presentation-format-specific bundles', () => {
    ...
    expect(device.createRenderPipeline).toHaveBeenCalledTimes(26);
  });
  ...
  it('never shares GPU resources across devices', () => {
    ...
    expect(firstDevice.createRenderPipeline).toHaveBeenCalledTimes(13);
    expect(secondDevice.createRenderPipeline).toHaveBeenCalledTimes(13);
  });
});
