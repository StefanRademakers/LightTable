...
describe('getCorePipelineBundle', () => {
  ...
  it('separates presentation-format-specific bundles', () => {
    ...
    expect(device.createRenderPipeline).toHaveBeenCalledTimes(30);
  });
  ...
  it('never shares GPU resources across devices', () => {
    ...
    expect(firstDevice.createRenderPipeline).toHaveBeenCalledTimes(15);
    expect(secondDevice.createRenderPipeline).toHaveBeenCalledTimes(15);
  });
});
