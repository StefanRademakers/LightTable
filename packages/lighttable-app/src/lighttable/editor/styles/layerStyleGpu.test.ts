...
describe('Layer Style GPU settings', () => {
  ...
  it('uses the retained dense field for non-jittered inner glows', () => {
    const effect = createDefaultLayerStyle('inner-glow');
    if (effect.kind !== 'inner-glow') throw new Error('Expected Inner Glow.');
    const stack = createDefaultLayerStyleStack();
    effect.size = 8;
    expect(layerStyleGaussianBlurPlan(effect, stack, 400, 200, 'final')).toEqual({
      scale: 1, workingWidth: 400, workingHeight: 200, workingRadius: 8
