...
export const BASIC_BRUSH_PRESETS: readonly BrushPresetDefinition[] = [
  {
...
const presetById = new Map(BASIC_BRUSH_PRESETS.map((preset) => [preset.id, preset]));

export const resolveBrushPreset = (id: BrushPresetId | string): BrushPresetDefinition =>
  presetById.get(id as BrushPresetId) ?? BASIC_BRUSH_PRESETS[0]!;
