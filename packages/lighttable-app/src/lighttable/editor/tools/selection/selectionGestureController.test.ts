...
describe('SelectionGestureController', () => {
  it('owns a rectangle gesture from begin through finish', () => {
    ...
    expect(controller.begin(7, 'select-rectangle', { x: 10, y: 20 })).toEqual({
      kind: 'rectangle',
    ...
    expect(controller.finish(7, { shiftKey: true, altKey: false })).toEqual({
      kind: 'apply',
      mode: 'add',
      shape: {
        kind: 'rectangle',
  ...
  it('samples free selections only after meaningful pointer movement', () => {
    const controller = new SelectionGestureController();
    controller.begin(2, 'select-free', { x: 0, y: 0 });
  ...
  it('clears on an invalid unmodified gesture but preserves on modified gestures', () => {
    const controller = new SelectionGestureController();
    controller.begin(3, 'select-ellipse', { x: 1, y: 1 });
    expect(controller.finish(3, { shiftKey: false, altKey: false })).toEqual({
      kind: 'clear'
    });
    controller.begin(4, 'select-ellipse', { x: 1, y: 1 });
    expect(controller.finish(4, { shiftKey: true, altKey: false })).toEqual({
      kind: 'none'
    });
  });
  ...
  it('ignores foreign pointers and supports explicit cancellation', () => {
    const controller = new SelectionGestureController();
    controller.begin(11, 'select-rectangle', { x: 0, y: 0 });
    expect(controller.move(12, { x: 10, y: 10 })).toBeNull();
    expect(controller.finish(12, { shiftKey: false, altKey: false })).toBeNull();
  ...
  it('does not leak mutable draft objects to callers', () => {
    const controller = new SelectionGestureController();
    const draft = controller.begin(1, 'select-rectangle', { x: 4, y: 5 });
