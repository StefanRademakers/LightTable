...
fn onePixelInnerContourCoverage(imageUv: vec2f) -> f32 {
  let offset = vec2f(
    1.0 / max(view.rectWidth, 1.0),
    1.0 / max(view.rectHeight, 1.0)
  );
  ...
  let isInnerBoundaryPixel = select(0.0, 1.0, center >= 0.5 && low < 0.5);
  return isInnerBoundaryPixel * smoothstep(0.015, 0.20, center - low);
}
...
fn main(input: VertexOutput) -> @location(0) vec4f {
  ...
  // The inner iso-contour occupies exactly one screen pixel at every zoom.
  // Do not add a second underlay: that makes the visual selection wider than
  // its canonical bounds even though the underlying mask remains unchanged.
  ...
  return vec4f(lineColor, line * 0.98);
}
`;
