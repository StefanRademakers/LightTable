 ...
 * @param {number} first_line_indent
 * @param {number} start_indent
 * @param {number} end_indent
 ...
 */
export function realize_flow_text(session_key, key, text, max_width, alignment, line_height_kind, line_height_value, first_line_indent, start_indent, end_indent, space_before, space_after, origin_x, origin_y, max_glyph_count, style_meta, style_metrics, font_strings_utf8, string_ranges) {
    const ptr0 = passStringToWasm0(session_key, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    ...
    const ret = wasm.realize_flow_text(ptr0, len0, ptr1, len1, ptr2, len2, isLikeNone(max_width) ? Number.MAX_SAFE_INTEGER : Math.fround(max_width), alignment, line_height_kind, line_height_value, first_line_indent, start_indent, end_indent, space_before, space_after, origin_x, origin_y, max_glyph_count, ptr3, len3, ptr4, len4, ptr5, len5, ptr6, len6);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
