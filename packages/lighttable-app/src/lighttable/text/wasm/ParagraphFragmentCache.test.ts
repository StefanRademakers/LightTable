...
describe('paragraph fragment cache', () => {
  it('reuses unchanged fragments and misses only an edited paragraph', () => {
    const cache = new ParagraphFragmentCache<{ bytes: number; value: string }>((value) => value.bytes, 400);
  ...
  it('evicts the least recently used fragment by retained bytes', () => {
    const cache = new ParagraphFragmentCache<{ bytes: number; value: string }>((value) => value.bytes, 172);
  ...
  it('does not retain an oversized fragment and releases all owned bytes', () => {
    const cache = new ParagraphFragmentCache<{ bytes: number }>((value) => value.bytes, 100);
    const oversized = { bytes: 101 };
    expect(cache.set('oversized', oversized)).toBe(oversized);
    expect(cache.get('oversized')).toBeUndefined();
    cache.set('kept', { bytes: 20 });
    cache.clear();
    expect(cache.metrics()).toMatchObject({ entries: 0, byteLength: 0, budgetBytes: 100 });
  });
});
