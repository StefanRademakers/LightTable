...
export function executeSemanticLayerStyleCommand(
  ...
  let result: StackResult | EffectResult | null = null;
  const changed = dependencies.changeDocument((before) => {
    ...
    if (command.kind === 'stack-update') {
      ...
      return applyLayerStyleSnapshot(before, layer.id, validated);
    }
    ...
    if (command.kind === 'add') {
      ...
      if (command.kind === 'remove') stack.effects.splice(index, 1);
        ...
        stack.effects[index] = { ...stack.effects[index], enabled: command.enabled };
      } else {
