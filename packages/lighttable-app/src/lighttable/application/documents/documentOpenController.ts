...
> {
  ...
  async open<Source = Blob>(
    ...
    const openInFlight = this.unsettledOpenCount > 0;
    ...
    try {
      ...
      if (result.status === "completed") {
        ...
        // A reused renderer may already have replaced or partially allocated its
        // document resources before hydration fails. It is no longer a valid
        // reuse candidate: retire it exactly like a failed fresh renderer.
        if (reusableRenderer && this.renderer === reusableRenderer) {
          this.retireRenderer(reusableRenderer);
        }
  ...
  private retireRenderer(renderer = this.renderer): void {
    if (!renderer || this.renderer !== renderer) return;
    const publishDiscard = this.rendererDiscard;
    this.renderer = null;
    this.rendererDiscard = null;
    try {
      publishDiscard?.(renderer);
    } finally {
      renderer.destroy();
    }
  }
